...
> **⚠️ Pre-WXT historical design doc.** The *behaviour* described here is current, but the **build
> system and file layout have since migrated from CRXJS to WXT** — the code wins wherever they differ.
> Key relocations: content scripts → `src/entrypoints/{main-world,bridge}.content.ts`; service worker →
> `src/entrypoints/background.ts`; offscreen/sandbox → `src/entrypoints/{offscreen,sandbox}/`; side panel
> → `src/entrypoints/sidepanel/`. The manifest is **generated by WXT** (`wxt.config.ts`) — there is no
> hand-written `manifest.config.ts` / `vite.config.ts`, and **no `web_accessible_resources`**. Build
> output is `.output/chrome-mv3`; package with `npm run zip`. Treat the CRXJS / `dist/` / hand-written-manifest
